...
class ClassroomTeacherView extends React.Component {
  ...
  componentDidMount() {
    ...
    //   getClassroomData()
    // ]).then((result) => {
    //   const { name, periods } = result[0].data();
    //   const { cardsMissedMost, averageRatingPerCard } = result[1];
    //   this.setState(() => ({
    //     name: name,
    //     periods: periods,
    //     cardsMissedMost: cardsMissedMost,
    //     averageRatingPerCard: averageRatingPerCard
    //   }));
    // }).catch((err) => {
    //   console.error(err);
    // })
